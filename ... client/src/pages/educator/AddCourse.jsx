...
const AddCourse = () => {
  ...
  return (
    ...
    md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-900'>
        ...
        <div className='flex items-center justify-between flex-wrap'>
          <div className='flex flex-col gap-1'>
            ...
            type='number' placeholder='0' className='outline-none md:py-2.5 py-2 w-28
            px-3 rounded border border-gray-500' required />
          </div>
          <div className='flex md:flex-row flex-col items-center gap-3'>
            <p>Course Thumbnail</p>
            <label htmlFor='thumbnailImage' className='flex items-center gap-3'>
              <img src={assets.fileupload} alt="" className='h-14 w-14 p-3 bg-blue-500 rounded'/>
          ...
          {chapters.map((chapter, chapterIndex)=>(
            <div key={chapterIndex} className='bg-white border rounded-lg mb-4'>
              ...
              <div className='flex items-center'>
                ...
                src={assets.down_arrow} width={14} alt='' 
                className={`mr-2 cursor-pointer transition-all
                ${chapter.collapsed && "-rotate-90"}`}/>

                <span className='font-semibold'>{chapterIndex+1}
              ...
              {!chapter.collapsed &&(
                ...
                {chapter.chapterContent.map((lecture,lectureIndex)=>(
                  <div key={lectureIndex} className='flex justify-between items-center mb-2'>
                    ...
                    <img src={assets.cross_icon} alt='' onClick={()=>handleLecture('remove',
                      chapter.chapterId,lectureIndex)} className='h-4 w-4 cursor-pointer'/>
                  </div>
                ))}
