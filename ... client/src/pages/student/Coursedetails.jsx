...
const Coursedetails = () => {
  ...
  return courseData ?(
    ...
    <div className='max-w-xl z-10 text-gray-500'>
      ...
      {/* review and ratings */}
         <div className='flex items-center sapce-x-2 pt-3 pb-1 text-sm'>
                ...
                <p className=''>{courseData.enrolledStudents.length} {courseData.enrolledStudents
                  .length > 1 ? 'students':'student'}</p>
                      ...
                      <div key={index} className='border border-gray-300 bg-white mb-2 rounded'>
                        ...
                        cursor-pointer select-none' onClick={()=> toggleSection(index)}>
                          <div className='flex items-center gap-2'>
                            <img src={assets.down_arrow} alt="arrow_icon" className= {
                            `w-4 h-4 transform transition-transform ${openSections[index] ? 'rotate-180 ' :''}`}/>
                            <p className='font-medium md:text-base text-sm'>
                              {chapter.chapterTitle}</p>
                          </div>
                        ...
                        <div className={`overflow-hidden transition-all
                         duration-300 ${openSections[index] ? 'max-h-96' : 'max-h-0'}`}>
                          <ul className='md:pl-10 pl-4 pr-4 py-2 text-gray-600 border-t border-gray-300'>
                            {chapter.chapterContent.map((lecture,i)=>(
                              <li key={i} className='flex items-start gap-2 py-1'>
                                <img src={assets.playarrow} alt='play icon' 
                                className='w-4 h-4 mt-1'/>
                                <div className='flex justify-between w-full gap-4 text-gray-800 text-xs md:text-default'>
                                  ...
                                  <div className='flex gap-3 items-center whitespace-nowrap"'>
                                    ...
                                    <p className="text-gray-500">{humanizeDuration(lecture.lectureDuration * 60)}</p>
                                  </div>
                                </div>
                              </li>
                            ))}
    ...
    md-rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[550px]">
      {
            playerData ? 
      ...
      <div className='p-5'>
        <div className='flex items-center gap-2'>
         ...
         <img className='w-3.5' src={assets.clock} alt='time left'/>
          

        <p className='text-black-500'><span className='font-medium'>
        5 days</span> left at this price!</p>
        </div>
        ...
        md:pt-4 text-gray-500'>
          ...
          <div className='flex items-center gap-1'>
            <img src={assets.clock} alt='clock' className='h-4 w-4'/>
            <p>{calculateCourseDuration(courseData)}</p>
          </div>

          <div className='h-4 w-px bg-gray-500/40'></div>
          <div className='flex items-center gap-1'>
            <img src={assets.lesson_icon} alt='clock' className='h-4 w-4'/>
            <p>{calculateNoOfLectures(courseData)} lessons</p>
          </div>

        </div>
         <button className='md:mt-6 mt-4 w-full py-3 rounded bg-blue-600 text-white font-medium'>
          {isAlreadyEnrolled ? 'Alresdy Enrolled' : 'Enroll Now'}
         </button>
